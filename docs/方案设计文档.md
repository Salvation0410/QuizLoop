# 《Quizeloop AI 闯关学习小程序》方案设计文档

## 1. 文档目标

本文档用于指导“Quizeloop AI 闯关学习小程序”从 0 到 1 完成 MVP 方案设计与实施落地，重点解决以下问题：

1. 明确小程序前端技术选型，并给出可执行的推荐结论。
2. 围绕核心业务闭环设计整体架构：用户输入内容 => AI 生成题目 => 用户答题 => AI 生成分析报告。
3. 给出后端技术方案、核心 Prompt 设计、接口协议、领域数据模型和分阶段开发路径。
4. 对需求文档中的待办项进行收敛，区分“现在就做”和“后续再做”，避免前期工程失控。

本文档默认遵循以下前提：

- 后端语言采用 Python。
- 后端框架采用 FastAPI。
- 大模型通过 **CloseAI 中转平台**调用，采用 OpenAI 兼容协议。
- P0 模型固定为 `gpt-5.4`；中转地址、密钥和超时均通过环境变量配置。
- LangChain/LangGraph 的具体接入方式参考 `D:\AI_Agent\langchain1.2_study_total` 中已有代码实现，并在本项目中封装为独立 Provider。
- MVP 不接联网搜索、微信登录、Milvus 和多模态，但 MySQL 从首版开始用于历史数据持久化。
...
1. 用户输入一句话、一段文本或一个主题。
2. 后端调用 AI 生成默认 10 道题目，允许配置 5-15 道，题型包含单选、多选、判断。
...
1. 微信授权登录、用户注册、账号体系。
2. 用户账户体系；MySQL 仅实现匿名会话、题库、答题、奖励和报告所需的最小持久化。
...
4. AI 适配层：通过 LangChain 统一封装 CloseAI 中转的 `gpt-5.4` 调用
...
4. 后端通过 LangChain 的 `ChatPromptTemplate` 组装提示词，并调用 CloseAI 中转的 `gpt-5.4`。
5. 后端校验 JSON 结构是否合法。
6. 后端返回标准题库数据给前端。
7. 前端进入闯关页开始答题。
...
本项目应当把 LangChain 作为 CloseAI 中转模型的正式接入层，而不是仅作为可选辅助库；LangGraph 负责题库和报告的节点编排。
...
3. `langchain-openai` 的 `ChatOpenAI` 支持通过 `base_url` 和 `api_key` 对接 OpenAI 兼容接口，因此可以直接接 CloseAI 中转平台。
4. 把模型调用统一放在 LangChain Provider 层，后续切换模型、增加重试、增加输出解析和引入 RAG 时，扩展路径更清晰。

使用边界也需要明确：

1. MVP 中使用 LangChain 的 `ChatPromptTemplate`、`ChatOpenAI`、`with_structured_output()`。
2. P0 使用 LangGraph 编排出题和报告的有限状态流程，但不引入 Agent、联网工具调用或 RAG。
3. LangChain/LangGraph 是 AI 编排层，仍要保留清晰的服务层与异常处理，不能把所有业务逻辑堆进图节点中。

结论是：**CloseAI 中转 + `gpt-5.4` 是 P0 模型方案，LangChain + LangGraph 是正式 AI 接入与任务编排方案。**
...
1. 基于 `langchain-openai` 初始化 CloseAI 中转对应的 `ChatOpenAI`
2. 统一管理模型、温度、超时、重试配置
...
后续模型切换不得散落在业务代码中，统一通过 `LLM_MODEL` 配置；切换前必须重新跑结构化输出、耗时、成本和题目质量回归测试。

推荐理由：

1. P0 先固定模型，便于建立可重复的 Prompt 和质量基线。
2. CloseAI 中转统一屏蔽底层供应商差异，客户端和业务层只依赖 OpenAI 兼容协议。
3. 后续可在 Provider 层增加模型路由，但不能改变题库和报告 Schema。

### 7.2 接入方式

使用 `LangChain + langchain-openai` 对接 CloseAI 中转平台的 OpenAI 兼容接口，推荐方式如下：

- 使用 `ChatOpenAI`
- `base_url = CLOSEAI_BASE_URL`（由 CloseAI 平台提供的兼容接口地址）
...
根据 CloseAI OpenAI 兼容接口和 LangChain 的能力，结构化输出按以下策略落地：

1. LangChain 层优先使用 `with_structured_output()` 约束输出 Schema。
2. Prompt 中仍然明确要求输出 JSON 语义，并给出目标字段结构。
3. CloseAI 中转底层保持 OpenAI 兼容调用方式，必要时补充 `response_format` 约束；以实际平台支持情况为准。
...
P0 从第一阶段就接入 MySQL，但只实现匿名会话所需的最小表和迁移：会话、生成任务、题库、题目、选项、答题记录、奖励流水和报告。MySQL 保存历史事实；LangGraph State 和进程内 JobStore 只保存单次任务的运行期状态，不替代数据库。
...
2. 出题 Prompt V1
3. CloseAI `gpt-5.4` JSON 输出解析与重试机制
4. 题库 JSON 校验逻辑
